...
number_of_anchor_boxes = len(constants.anchor_boxes[0])
...
class Scale26(nn.Module):
    def __init__(self, number_of_classifications):
        super().__init__()
        self.number_of_classes = number_of_classifications
        self.layers = nn.Sequential(
            nn.Conv2d(384, 256, 3,padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(256, (number_of_classifications + 5) * number_of_anchor_boxes, 1),
        )

    def forward(self, x):
        return self.layers(x).reshape(x.shape[0], number_of_anchor_boxes, (self.number_of_classes + 5), x.shape[2], x.shape[3]).permute(0, 1, 3, 4, 2)
...
class Scale13(nn.Module):
    def __init__(self, number_of_classifications):
        ...
        self.layers = nn.Sequential(
            nn.Conv2d(256, 512, 3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(512, (number_of_classifications + 5) * number_of_anchor_boxes, 1),
        )
...
class YOLOV3Tiny(nn.Module):
    def __init__(self, in_channels=1, number_of_classes=constants.number_of_classes):
        ...
        self.block2 = Block2()
        self.block3 = Block3()
        self.scale13 = Scale13(number_of_classes)
        self.scale26 = Scale26(number_of_classes)
